...
import { CircleBlueGradient } from './CircleBlueGradient';
import { CircleTouchable } from './CircleTouchable';
...
export default class ReactNativeRingPicker extends React.Component {
    ...
    createFinishAnimationPromisesAndResolveIfIconsAreNotMovingAlready() {
        this.state.icons.forEach(icon => {
            this.ALL_ICONS_FINISH_ANIMATIONS.promises[icon.id] = new Promise(
                resolve => (this.ALL_ICONS_FINISH_ANIMATIONS.resolvers[icon.id] = resolve)
            );
    ...
    snapNearestIconToVerticalAxis(lastGesture) {
        let { minDistanceToVerticalAxis, minDistanceToHorizontalAxis, sign, currentSnappedIcon } =
            this.getMinDistanceToVerticalAxisAndSnappedIcon();
        [minDistanceToVerticalAxis, minDistanceToHorizontalAxis] = this.updateMinimalDistanceExponentialDeflection(
    ...
    getMinDistanceToVerticalAxisAndSnappedIcon() {
        ...
        let yCoordinateFromCssStyling = s.iconContainer.top - SQUARE_DIMENSIONS.ICON_PADDING_FROM_WHEEL;

        this.state.icons.forEach(icon => {
            let iconXCenterCoordinate = icon.position.x.__getValue() + s.icon.width / 2;
            let iconYCenterCoordinate = icon.position.y.__getValue();

            let distanceToXAxis = Math.abs(
                iconXCenterCoordinate - (this.state.XY_AXES_COORDINATES.X - this.state.XY_AXES_COORDINATES.PAGE_X)
            );
            ...
            if (distanceToXAxis <= minDistanceToVerticalAxis) {
                if (iconXCenterCoordinate > this.state.XY_AXES_COORDINATES.X - this.state.XY_AXES_COORDINATES.PAGE_X) {
                    ...
                    this.state.XY_AXES_COORDINATES.X - this.state.XY_AXES_COORDINATES.PAGE_X
                ) {
                    sign = 1;
                } else {
                    sign = 0;
    ...
    updateMinimalDistanceExponentialDeflection(
        ...
        let currentAngle =
            270 + this.state.CURRENT_ICON_SHIFT + (this.INDEX_EXTRACTORS[id] || 0) + index * this.ICON_POSITION_ANGLE;
    ...
    defineAxesCoordinatesOnLayoutDisplacement = () => {
        this._wheelNavigator.measure((x, y, width, height, pageX, pageY) => {
            this.setState({
                ...this.state,
                ICON_PATH_RADIUS: height / 2 + s.icon.height / 2 + SQUARE_DIMENSIONS.ICON_PADDING_FROM_WHEEL,
                XY_AXES_COORDINATES: {
                    X: pageX + width / 2,
                    Y: pageY + height / 2,
                    PAGE_Y: pageY,
    ...
    calculateIconCurrentPosition(icon) {
        ...
        return {
            top:
                ...
                this.state.ICON_PATH_RADIUS * Math.sin(currentIconAngle * (Math.PI / 180)),
            left:
                this.state.XY_AXES_COORDINATES.X -
                this.state.XY_AXES_COORDINATES.PAGE_X -
                s.icon.width / 2 +
                this.state.ICON_PATH_RADIUS * Math.cos(currentIconAngle * (Math.PI / 180)),
        };
    }
    ...
    calculateCurrentIconAngle(icon) {
        ...
        let currentAngle =
            270 + this.state.CURRENT_ICON_SHIFT + this.INDEX_EXTRACTORS[id] + index * this.ICON_POSITION_ANGLE;
        ...
        return (
            ...
            >
                ...
                >
                    ...
                    <View style={s.wheelTouchableCenter}>
